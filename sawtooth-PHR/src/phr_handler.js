...
const { phrState }=require('./state')
const cbor= require('cbor');
const crypto = require('crypto');
const FAMILY_NAME = "phr-family", VERSION = ["1.0"], NAMESPACE = ["phr", "phrfam", hash(FAMILY_NAME).substring(0,6)]
...
class PhrHandler extends TransactionHandler {
	...
	let PhrState= new phrState(context)
		...
		//console.log(context)
                ...
                switch(action){
                    case "upload":
	                address = NAMESPACE[2] + hash(update.pid).substring(0,6)+ hash(update.category).substring(0,6) + hash(update.category).substring(0,6);
                        let entries = {
                            [address] : cbor.encode(update)
                        }
												context.setState(entries);
                        //PhrState.setRecord(address, createdRecord);
...
*/
			...
			var demo =PhrState.getRecord(JSON.stringify(readentries)).data
			console.log(demo)
